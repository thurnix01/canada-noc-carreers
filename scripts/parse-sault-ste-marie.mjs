/**
 * Sault Ste. Marie RCIP parsers (HTML priority NOC table + WP REST employers).
 * Usage: node scripts/parse-sault-ste-marie.mjs
 */
import { mkdirSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';

const COMMUNITY_ID = 'sault-ste-marie';
const YEAR = '2026';
const NOC_URL = 'https://welcometossm.com/rcip_employer/';
const EMPLOYERS_API = 'https://welcometossm.com/wp-json/wp/v2/pages?slug=designated-employers';
const EMPLOYERS_URL = 'https://welcometossm.com/designated-employers/';
...
function isJunkEmployerLine(text) {
  const t = String(text || '').trim();
  if (!t || t.length < 3) return true;
  if (/^(&nbsp;|\s)*$/i.test(t)) return true;
  if (/^below is a list|please note|important:|you should never pay|designation of employers|only employers in the selected/i.test(t))
    return true;
  if (/^en francaise$/i.test(t)) return true;
  return false;
}

export function parseEmployersFromRendered(html, nowIso = new Date().toISOString()) {
  const out = [];
  const seen = new Set();
  ...
  while ((m = pRe.exec(html))) {
    let raw = m[1];
    if (/<a[^>]+href/i.test(raw) && stripTags(raw).length < 5) continue;

    let text = decodeEntities(raw.replace(/<[^>]+>/g, ' ')).replace(/\s+/g, ' ').trim();
    if (isJunkEmployerLine(text)) continue;

    let notes = '';
    const yearTag = text.match(/\((20\d{2})\)/);
    if (yearTag) {
      notes = `Designated year tag: ${yearTag[1]}`;
      text = text.replace(/\(\s*20\d{2}\s*\)/g, '').trim();
    }

    let recruiting_status = 'unknown';
    if (/\(Not currently hiring\)/i.test(text)) {
      recruiting_status = 'not_hiring';
      text = text.replace(/\(Not currently hiring\)/gi, '').trim();
    } else if (/\(not currently hiring\)/i.test(text)) {
      recruiting_status = 'not_hiring';
      text = text.replace(/\(not currently hiring\)/gi, '').trim();
    }
...
async function main() {
  ...
  const nocHtml = await nocRes.text();
  const nocs = parsePriorityNocs(nocHtml);

  const empRes = await fetch(EMPLOYERS_API, opts);
  const empData = await empRes.json();
  const rendered = empData?.[0]?.content?.rendered || '';
  const employers = parseEmployersFromRendered(rendered);
  console.log(`NOC HTTP ${nocRes.status}: ${nocs.length} NOCs`);
  console.log(`Employers API HTTP ${empRes.status}: ${employers.length} employers`);
...
const isDirect = process.argv[1] && process.argv[1].endsWith('parse-sault-ste-marie.mjs');
if (isDirect) {
  main().catch((err) => {
    console.error(err);
    process.exit(1);
